...
- **`@Nested`로 메서드별 그룹핑**: 테스트 클래스는 대상 메서드마다 `@Nested` 클래스로 나누고, 한글 `@DisplayName`을 붙인다. 컨트롤러/서비스/통합/JPA 테스트 모두 동일하게 적용.

---

## 컨트롤러 테스트

- `@WebMvcTest(controllers = X.class)` + 필요한 설정만 `@Import({...})` + `@ActiveProfiles("test")`
- `@MockBean`으로 Service/Facade 목킹
- 인증이 필요하면 `@WithMockAuthenticationPrincipal(role = "SELLER")` 사용
- 검증은 `mockMvc.perform().andExpect(jsonPath(...))` 체이닝

## 비즈니스 단위/mock 테스트 (`*ServiceUnitTest`)

- `@ExtendWith(MockitoExtension.class)` + `@Mock`(의존성) + `@InjectMocks`(대상)
- 별도 베이스 클래스 없음
- 검증은 공통 규칙대로 `assertThat(result)...` + `then(mock).should()...`

## 비즈니스 통합 테스트 (`*FacadeIntegrationTest` / `*IntegrationTest`)

- `@SpringBootTest` + `@ActiveProfiles("test")` + `@Transactional`(자동 롤백)
- S3 연동이 있으면 `S3IntegrationTestSupport`를 상속
- Mock 없이 실제 빈 조합으로 동작시키고, Fixture로 만든 엔티티를 Repository에 `saveAndFlush`로 저장한 뒤 대상 Facade/Service를 실행

## Data JPA 테스트 (`*RepositoryTest`)

- `@DataJpaTest` + `@AutoConfigureTestDatabase(replace = Replace.NONE)`(임베디드로 교체하지 않고 TestContainers 실DB 사용)
- `@Import({TestContainersConfig.class, QueryDslConfig.class, ...})` + `@Transactional` + `@ActiveProfiles("test")`
- `TestContainersConfig`가 static 블록으로 MariaDB/Redis/LocalStack을 1회만 기동하고 `@ServiceConnection`으로 연결하므로, 새 베이스 클래스를 만들지 말고 이 설정을 `@Import`만 하면 됨
