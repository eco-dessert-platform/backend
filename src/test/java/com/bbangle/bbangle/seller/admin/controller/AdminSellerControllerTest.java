...
class AdminSellerControllerTest {
    ...
    class RejectSellerApplicationsTest {
        ...
        @DisplayName("ADMIN 권한이 없는 사용자는 접근이 거부된다.")
        void fail_rejectSellerApplications_forbidden() throws Exception {
        ...
        @Test
        @WithMockUser(roles = "ADMIN")
        @DisplayName("일부 거절 실패 시 failDetails에 실패 정보가 담긴다.")
        void rejectSellerApplications_partial_failure() throws Exception {

            // given
            List<Long> ids = List.of(1L, 999L);
            AdminSellerRequest.StoreApplicationIds request = new AdminSellerRequest.StoreApplicationIds(ids);

            AdminSellerApplicationRejectList.FailDetail failDetail = AdminSellerApplicationRejectList.FailDetail.builder()
