...
class AdminSellerServiceIntegrationTest {
    ...
    class RejectStoreApplicationsTest {
        ...
        @Test
        @DisplayName("빈 id 목록 입력 시 빈 결과를 반환한다")
        ...
        @Test
        @DisplayName("성공, 미존재, 이미 승인이 혼합된 경우 각각 올바르게 처리된다")
        void rejectStoreApplications_mixed_result() {

            // given
            Seller sellerPending = sellerRepository.save(SellerFixture.defaultSeller());
            Seller sellerApproved = sellerRepository.save(SellerFixture.defaultSeller(CertificationStatus.APPROVED));

            StoreApplication pendingApp = storeApplicationRepository.save(
                StoreApplicationFixture.defaultStoreApplication(sellerPending, null)
            );
            StoreApplication approvedApp = storeApplicationRepository.save(
                StoreApplicationFixture.defaultStoreApplication(DEFAULT_STORE_NAME, sellerApproved, StoreApprovalStatus.APPROVE)
            );
            em.flush();
            em.clear();

            long nonExistentId = 999999L;
            List<Long> ids = List.of(pendingApp.getId(), approvedApp.getId(), nonExistentId);
