...
class AdminSellerServiceUnitTest {
    ...
    class RejectStoreApplicationsTest {
        ...
        @Test
        @DisplayName("빈 ID 목록을 전달하면 성공/실패 모두 빈 결과를 반환한다")
        ...
        @Test
        @DisplayName("성공, 미발견, 비즈니스 예외가 혼재할 때 각각 올바르게 분류된다")
        void rejectStoreApplications_mixed_results() {
            ...
            StoreApplication app1 = mock(StoreApplication.class);
            StoreApplication app3 = mock(StoreApplication.class);

            given(app1.getId()).willReturn(1L);
            given(app3.getId()).willReturn(3L);
            // id=2 is not in the repository result (not found)
            given(storeApplicationRepository.findAllWithSellerByIdIn(ids)).willReturn(List.of(app1, app3));
            willThrow(new BbangleException(BbangleErrorCode.REQUEST_IS_APPROVED)).given(app3).reject();
            ...
            assertThat(result.failDetails()).anySatisfy(d -> {
        ...
        @Test
        @DisplayName("중복 ID 입력 시 각각 별도로 처리된다")
        void rejectStoreApplications_duplicate_ids() {
            ...
            // then - both iterations attempt to process id=1; first succeeds
            assertThat(result.successIds()).contains(1L);
            // total processed = 2 (success + fail or two successes)
            assertThat(result.successIds().size() + result.failDetails().size()).isEqualTo(2);
        }
    }
}