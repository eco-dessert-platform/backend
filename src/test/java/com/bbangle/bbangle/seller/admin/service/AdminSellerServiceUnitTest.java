...
class AdminSellerServiceUnitTest {
    ...
    class RejectStoreApplicationsTest {
        ...
        @Test
        @DisplayName("성공, NOT_FOUND 실패, 비즈니스 예외 실패가 혼합된 경우 각각 독립적으로 처리된다")
        void rejectStoreApplications_mixed_results() {

            // given
            List<Long> ids = List.of(1L, 2L, 3L);

            StoreApplication successApp = mock(StoreApplication.class);
            StoreApplication failApp = mock(StoreApplication.class);

            given(successApp.getId()).willReturn(1L);
            given(failApp.getId()).willReturn(3L);
            willThrow(new BbangleException(BbangleErrorCode.REQUEST_IS_APPROVED)).given(failApp).reject();

            // id=2 is missing from repository (NOT_FOUND)
            given(storeApplicationRepository.findAllWithSellerByIdIn(ids)).willReturn(List.of(successApp, failApp));
            ...
            assertThat(result.failDetails()).anyMatch(
                d -> d.storeApplicationId() == 2L && d.reason().equals(BbangleErrorCode.NOT_FOUND_REQUEST.getMessage())
            );
            assertThat(result.failDetails()).anyMatch(
                d -> d.storeApplicationId() == 3L && d.reason().equals(BbangleErrorCode.REQUEST_IS_APPROVED.getMessage())
            );
        }

        @Test
        @DisplayName("모든 ID가 존재하지 않으면 모두 실패로 처리된다")
