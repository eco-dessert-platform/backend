...
class AdminStoreControllerTest {
    ...
    class ApproveStoreNameTest {
        ...
        @Test
        @DisplayName("스토어명 변경 요청 승인 시, 요청이 존재하지 않으면 404를 반환한다.")
        @WithMockUser(roles = "ADMIN")
        ...
        @Test
        @DisplayName("스토어명 변경 요청 승인 시, 이미 거절된 요청이면 400을 반환한다.")
        @WithMockUser(roles = "ADMIN")
        void fail_approveStoreName_alreadyRejected() throws Exception {

            // given
            long requestId = 1L;

            given(adminStoreService.approveStoreName(requestId))
                .willThrow(new BbangleException(BbangleErrorCode.REQUEST_IS_REJECTED));
            ...
            mockMvc.perform(patch(AdminApiPath.PREFIX + "/stores/{requestId}/approve", requestId)
                ...
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.success").value(false))
                .andExpect(jsonPath("$.code").value(BbangleErrorCode.REQUEST_IS_REJECTED.getCode()))
                .andExpect(jsonPath("$.message").value(BbangleErrorCode.REQUEST_IS_REJECTED.getMessage()));
        }

        @Test
        @DisplayName("ADMIN 권한이 없으면 스토어명 변경 요청 승인에 실패한다.")
    ...
    class RejectStoreNameTest {
        ...
        @Test
        @DisplayName("스토어명 변경 요청 거절 시, category가 null이면 400 유효성 오류를 반환한다.")
        @WithMockUser(roles = "ADMIN")
        void fail_rejectStoreName_nullCategory() throws Exception {

            // given
            long requestId = 1L;
            // category is null - violates @NotNull
            String requestBody = "{\"category\": null, \"rejectDetail\": \"부적절한 이름\"}";
        ...
        @Test
        @DisplayName("스토어명 변경 요청 거절 시, 이미 승인된 요청이면 400을 반환한다.")
        @WithMockUser(roles = "ADMIN")
        void fail_rejectStoreName_alreadyApproved() throws Exception {

            // given
            long requestId = 1L;
            UpdateStoreNameRejectRequest request =
                new UpdateStoreNameRejectRequest(StoreNameRejectCategory.ETC, "부적절한 이름");

            given(adminStoreService.rejectStoreName(eq(requestId), any(UpdateStoreNameRejectRequest.class)))
                .willThrow(new BbangleException(BbangleErrorCode.REQUEST_IS_APPROVED));
            ...
            mockMvc.perform(patch(AdminApiPath.PREFIX + "/stores/{requestId}/reject", requestId)
                ...
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.success").value(false))
                .andExpect(jsonPath("$.code").value(BbangleErrorCode.REQUEST_IS_APPROVED.getCode()))
                .andExpect(jsonPath("$.message").value(BbangleErrorCode.REQUEST_IS_APPROVED.getMessage()));
        }
    }
}