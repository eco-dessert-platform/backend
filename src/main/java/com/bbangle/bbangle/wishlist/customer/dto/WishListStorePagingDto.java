...
public class WishListStorePagingDto {
    ...
    public static WishListStorePagingDto of(Page<WishListStoreResponseDto> wishStorePaging) {
        if (!wishStorePaging.hasNext()) {
            return WishListStorePagingDto.getLastPage(wishStorePaging.getContent(),
    ...
    private static WishListStorePagingDto getLastPage(
        List<WishListStoreResponseDto> wishStorePaging, long lastPage) {
        return newPagingHasNext(wishStorePaging, lastPage, LAST_PAGE);
    }

    private static WishListStorePagingDto newPagingHasNext(
        List<WishListStoreResponseDto> wishStorePaging, long lastPage, long nextPage) {
