...
public class AnalyticsService {
    ...
    @Transactional(readOnly = true)
    public AnalyticsMembersCountResponseDto countMembersByPeriod(Optional<LocalDate> startDate,
        ...
        return AnalyticsMembersCountResponseDto.from(
            memberRepository.countMembersCreatedBetweenPeriod(startLocalDate, endLocalDate));
    }


    @Transactional(readOnly = true)
    public AnalyticsCreatedWithinPeriodResponseDto analyzeWishlistBoardByPeriod(
        Optional<LocalDate> startDate, Optional<LocalDate> endDate) {
        LocalDate startLocalDate = startDate.orElse(LocalDate.now().minusDays(6));
        LocalDate endLocalDate = endDate.orElse(LocalDate.now());

        List<DateAndCountDto> rawResults = wishListBoardRepository.countWishlistCreatedBetweenPeriod(
            startLocalDate, endLocalDate);

        List<DateAndCountDto> results = mapResultsToDateRangeWithCount(startLocalDate, endLocalDate,
            rawResults,
    ...
    @Transactional(readOnly = true)
    public AnalyticsCreatedWithinPeriodResponseDto analyzeReviewByPeriod(
        Optional<LocalDate> startDate, Optional<LocalDate> endDate) {
        LocalDate startLocalDate = startDate.orElse(LocalDate.now().minusDays(6));
        LocalDate endLocalDate = endDate.orElse(LocalDate.now());

        List<DateAndCountDto> rawResults = reviewRepository.countReviewCreatedBetweenPeriod(
            startLocalDate, endLocalDate);
    ...
    @Transactional(readOnly = true)
    public List<AnalyticsCumulationResponseDto> countCumulatedReviewsByPeriod(
        Optional<LocalDate> startDate, Optional<LocalDate> endDate) {
        LocalDate startLocalDate = startDate.orElse(LocalDate.now().minusDays(6));
        LocalDate endLocalDate = endDate.orElse(LocalDate.now());

        List<AnalyticsCumulationResponseDto> rawResults = reviewRepository.countCumulatedReviewBeforeEndDate(
            startLocalDate, endLocalDate);

        return mapResultsToDateRangeWithCumulativeCount(startLocalDate, endLocalDate, rawResults,
            AnalyticsCumulationResponseDto::date, AnalyticsCumulationResponseDto::count,
            AnalyticsCumulationResponseDto::new);
    }
    ...
    private <T, R> List<R> mapResultsToDateRangeWithCount(
        ...
        return dateRange.stream()
            .map(date -> constructor.apply(Date.valueOf(date),
                mapResults.getOrDefault(Date.valueOf(date), 0L)))
            .toList();
    }
    ...
    private <T, R> List<R> mapResultsToDateRangeWithCumulativeCount(
        ...
        for (LocalDate localDate = startLocalDate; !localDate.isAfter(endLocalDate);
