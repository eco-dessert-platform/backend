...
/**
 * 일별 정산 내역을 엑셀(.xlsx)로 작성하는 컴포넌트.
 * SXSSFWorkbook을 사용해 스트리밍 방식으로 작성하므로 대용량 데이터에도 OOM이 발생하지 않는다.
 */
@Component
public class DailySettlementExcelWriter {

    /** 엑셀 시트 이름 */
    private static final String SHEET_NAME = "일별 정산 내역";
    ...
    /**
     * 정산 내역 목록을 엑셀로 작성해 outputStream에 직접 출력한다.
     * 데이터가 0건이면 헤더만 작성된 빈 엑셀을 반환한다.
     *
     * @param rows         엑셀에 작성할 정산 내역 목록
     * @param outputStream 응답에 사용할 출력 스트림
     */
    public void write(List<DailySettlementSummary> rows, OutputStream outputStream) throws IOException {
     ...
     */
    private void writeBody(Sheet sheet, List<DailySettlementSummary> rows, CellStyle moneyStyle) {
        DailySettlementExcelColumn[] columns = DailySettlementExcelColumn.values();

        // 헤더 다음 행(1번 인덱스)부터 데이터 작성
        for (int rowIdx = 0; rowIdx < rows.size(); rowIdx++) {
            Row row = sheet.createRow(rowIdx + 1);
            DailySettlementSummary summary = rows.get(rowIdx);
    ...
    private void setColumnWidths(Sheet sheet) {
        for (int i = 0; i < DailySettlementExcelColumn.values().length; i++) {
