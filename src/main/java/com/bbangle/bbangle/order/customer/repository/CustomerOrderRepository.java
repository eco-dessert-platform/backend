...
/**
 * 소비자 주문목록 조회 전용 QueryDSL 리포지토리. 판매자 조회 로직({@code OrderDSLRepository})과 분리하여 소비자 관점 쿼리만 담습니다. 현재는 별도 필터 없이 인증 회원의 주문을
 * 주문일 최신순으로 페이징 조회합니다.
 */
@Repository
...
public class CustomerOrderRepository {
    ...
    /**
     * 회원의 주문을 주문일(orderDate) 최신순으로 페이징 조회합니다. fetchJoin + 페이징 시 카테시안 곱 문제를 피하기 위해 2단계로 조회합니다. 1) ID만 페이징 조회 → 2) 해당 ID로
     * orderItems/product/board/payment fetchJoin 조회 후 정렬 순서 복원
     */
     ...
     * 카테시안 곱 없이 한 번의 쿼리로 주문상품/상품/게시글/스토어/결제를 로딩합니다. 배송정보(OrderDelivery)는 주문상품 1:N 컬렉션이라 함께 fetchJoin 하지 않고 서비스 계층에서
     * {@code findLatestByOrderItemIds} 로 별도 일괄 로딩하여 N+1 을 방지합니다.
     */
    ...
    /**
     * 결제 전(PAYMENT_PENDING) 주문을 목록에서 제외한다. 주문상품이 없는 주문은 기존 동작대로 목록에 남긴다.
     */
